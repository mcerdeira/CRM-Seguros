...
(function (ClientUtility) {
    var CustomerExperienceSurveyUtil = (function () {
        ...
        CustomerExperienceSurveyUtil.TriggerEventAndHandleSurvey = function (teamName, eventName, surveyName, optionalParameters) {
            ...
            CustomerExperienceSurveyUtil.HandlePromise(CustomerExperienceSurveyUtil.HandleCESAction(param), optionalParameters, eventParamArray);
        };
        ...
        CustomerExperienceSurveyUtil.HandlePromise = function (CESActionPromise, optionalParameters, telemetryEventParamArray) {
            var _this = this;
            CESActionPromise.then(function (cesResult) {
                if (cesResult) {
                    try {
                        var formsProId = cesResult["FormsProId"];
                        var se = new window.top.SurveyEmbed(formsProId, "https://customervoice.microsoft.com/", "https://mfpembedcdnmsit.azureedge.net/mfpembedcontmsit/", "true");
                        if (optionalParameters.renderDimensions)
                            se.renderPopup(CustomerExperienceSurveyUtil.GenerateFormContext(cesResult["AADID"], optionalParameters), optionalParameters.renderDimensions.width, optionalParameters.renderDimensions.height);
                        else
                            se.renderPopup(CustomerExperienceSurveyUtil.GenerateFormContext(cesResult["AADID"], optionalParameters));
                        ...
                        Xrm.Reporting.reportFailure(_this.componentName, { name: "Failed to Render Survey", message: e.message }, undefined, telemetryEventParamArray);
                    }
                }
            }).catch(function (reason) {
                Xrm.Reporting.reportFailure(_this.componentName, { name: "CESService Rejected", message: reason }, undefined, telemetryEventParamArray);
            });
        };
        CustomerExperienceSurveyUtil.GenerateFormContext = function (AADID, optionalParameters) {
            var context;
            if (!optionalParameters.context)
