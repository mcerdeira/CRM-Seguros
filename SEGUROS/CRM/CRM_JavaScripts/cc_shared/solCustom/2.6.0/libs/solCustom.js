...
function initializeSolLibrary($, window, document) {
    ...
    SearchableOptionList.prototype = {
        ...
        _onPointerLeave: function() {
            ...
            if (!this.$container.hasClass("sol-focused")) {
        ...
        _onPointerDown: function() {
            ...
            this.$container.addClass("sol-active sol-focused");
        },
        ...
        _deactivateControl: function(e) {
            ...
            this.$container.removeClass("sol-active sol-focused");
        },

        _collapseSelectedItemsContainer: function() {
            this.$currentSelectionContainer.scrollTop(0);
            this.$selectedItemsContainer.removeClass("sol-selecteditems-expanded");
            this._updateExpandSelectedItemsButtonText();
        },

        _expandSelectedItemsContainer: function() {
            this.$selectedItemsContainer.addClass("sol-selecteditems-expanded");
        ...
        _deactivateOtherControls: function(e) {
            // what was clicked?
            var $target = $(e.target);
            var $closestInnerContainer = $target.closest(".sol-inner-container");
            var $closestSolContainer = $target.closest(".sol-container");

            // clicking within the active control's inner container; no need to do anything
            if ($closestInnerContainer.length && $closestSolContainer.hasClass("sol-open")) {
                return;
            }

            // collapse any open dropdown
            // there should only be one, but let's iterate through the collection to be sure
            $(".sol-container.sol-open")
            ...
            $(".sol-container.sol-active.sol-focused")
                .not($closestSolContainer)
                .each(function(index, item) {
                    $(item).data(SearchableOptionList.prototype.DATA_KEY).deactivate();
                });
        },
        ...
        _initializeUiElements: function () {
            ...
            this.$noResultsItem = $(noresultsContainer.replace('{0}', (this.config.key + "_no-results")))
                .text(this.config.texts.noItemsAvailable).addClass("sol-hidden");

            // dropdown toggle button / caret
            var $caretContainer = $("<div class='sol-caret-container' />");
            this.$caret = $('<button class="sol-caret-button" tabindex="-1"><span class="sol-glyph" /></button>')
            ...
            // filter container
            var $filterContainer = $("<div class='sol-filter-container' />");
            ...
            // the dropdown itself
            this.$selection = $('<ul class="sol-selected-items sol-selection" role="listbox" tabindex="-1" />')
            ...
            // main container
            var solContainer = '<div id={0}_i class="sol-container" />';
            this.$container = $(solContainer.replace('{0}', this.config.key))
                .data(this.DATA_KEY, this)
                .toggleClass("sol-active sol-focused", this.config.alwaysActive)
                .append(this.$innerContainer)
                .insertBefore(this.$originalElement);

            // selected items containers (the items and toggle button)
            this.$selectedItemsContainer = $("<div class='sol-selecteditems-container' />");
            this.$selectedItemsToggleContainer = $("<div class='sol-selecteditems-toggle-container' />");

            // the +X/less toggle button
            // this should not be visible until we can determine the number of hidden selected items (hence the '.sol-invisible' class)
            // the button will always be aria-hidden, however, since the selected items list is always available to screen readers
            this.$selectedItemsToggleButton = $("<button class='sol-selecteditems-toggle sol-invisible' tabindex='-1' aria-hidden='true' />")
            ...
            this.$currentSelectionContainer = $("<ul class='sol-selecteditems sol-current-selection-normal' />")
                ...
                .on("focus", function(e) {
                    ...
                    el.children().removeClass("sol-selecteditem-active");
                    ...
                    if (!target.hasClass("sol-visible") && !self.$selectedItemsContainer.hasClass("sol-selecteditems-expanded")) {
                    ...
                    target.addClass("sol-selecteditem-active");
                    el.attr("aria-activedescendant", activeDescendant);
                })
                .on("blur", function() {
                   // remove the focused class
                    $(this).removeClass("sol-selecteditems-focused");
                });
            ...
            this.$viewmodeContainer = $("<div class='sol-viewmode-container' />");

            // viewmode selected items content & inner text container
            this.$viewmodeContent = $("<div class='sol-viewmode-content' />");
            this.$viewmodeText = $("<div class='sol-viewmode-text' />");
            ...
            this.$viewmodeButtonContainer = $("<div class='sol-viewmode-button-container' />");
            // this button will always be aria-hidden since the list of items is always available to screen readers
            this.$viewmodeExpansionToggle = $("<button class='sol-viewmode-button' aria-hidden='true'><span class='sol-glyph' /></button>")
                .attr("aria-label", this.config.texts.viewmodeExpandLabel)
                .on("click", function(e) {
                    self.$viewmodeContainer.toggleClass("sol-viewmode-expanded");

                    // update label
                    var button = $(this);
                    if (self.$viewmodeContainer.hasClass("sol-viewmode-expanded")) {
            ...
            if (this.config.multiple) {
                ...
                if (this.config.showSelectAll) {
                    var selectAllString = ('<input type="checkbox" class="sol-checkbox" id={0} tabindex="-1">').replace('{0}', this.config.key + "_selectAll");
                    this.$selectAllCheckbox = $(selectAllString)
                        ...
                        .on("focus", function() {
                            var $element = $(this);
                            $element.parent().addClass("sol-option-focused");
                        ...
                        .on("blur", function() {
                            var $element = $(this);
                            $element.parent().removeClass("sol-option-focused");
                        ...
                        .attr('aria-label', this.config.texts.selectAll);

                    var selectAllLabelElement = $(this.formatString('<label name={0} class="sol-label sol-selectall sol-option" for={1}>', this.config.key + "sol-label", this.config.key + "_selectAll"));
                    var selectAllDiv = ('<div class="sol-optionitem-text">{0}</div>').replace('{0}', this.config.texts.selectAll);
                    var numberOfItemsDiv = this.formatString('<div id={0} class="sol-itemcount-text">{1}</div>', this.config.key + "sol-no-of-Items", this.formatString(this.config.texts.totalItems, this.config.data.length));
                    this.$numberOfItemsDiv = $(numberOfItemsDiv);
                    var solLabelDivContainer = $('<div class="sol-label-text"></div></label>').append(selectAllDiv).append(this.$numberOfItemsDiv);
            ...
            // inputs to handle tab capturing
            this.$leadingInput = $("<input type='text' class='sol-tab-stop sol-leading-input' />");
            this.$trailingInput = $("<input type='text' class='sol-tab-stop sol-trailing-input' />");
            this.$container
                .prepend(this.$leadingInput)
                .append(this.$trailingInput);
            ...
            this.$alertContainer = $('<div class="sol-alert-text" aria-live="assertive" aria-atomic="true" aria-relevant="additions" />');
        ...
        _scrollSelectedItemIntoView: function($element) {
            ...
            if (!this.$selectedItemsContainer.hasClass("sol-selecteditems-expanded")) {
                return;
            }
        ...
        _initializeInputEvents: function () {
            ...
            // keyboard navigation
            // two elements are used to capture focus:
            // one to handle focus coming from above (the leading input), the other
            // to handle focus coming in from below (the trailing input);
            // both of these elements are hidden when the control is active; they're workarounds for the
            // particular nature of the control -- when in a "rest" state, it appears completely different
            // than when it's in an active state: indeed, at rest, the only thing visible is plain ol' text
            this.$leadingInput.on("focus", function(e) {
                self._getDefaultFocusElement().focus();
            });

            this.$trailingInput.on("focus", function(e) {
                self._getDefaultFocusElement(true).focus();
            });
        ...
        _handleEscape: function(e) {
            ...
            if (activeElement.is(this.$currentSelectionContainer)) {
                if (this.$selectedItemsContainer.hasClass("sol-selecteditems-expanded")) {
                    ...
                    if (!$(activeDescendant).hasClass("sol-visible")) {
                        this._updateCurrentlySelectedItem(this._getVisibleSelectedItems().first());
                    }
        ...
        _handleArrows: function(e) {
            ...
            var menuitemFocused = activeElement.hasClass("sol-checkbox");
            ...
            if (inputFocused) {
                ...
                if (this.$noResultsItem.hasClass("sol-hidden") && direction === 1) {
                    if (this.isClosed()) {
                        this.open();
                    }
            ...
            if (selectedItemsFocused) {
                ...
                if (!target.hasClass("sol-visible") && !this.$selectedItemsContainer.hasClass("sol-selecteditems-expanded")) {
                    this._toggleSelectedItemsContainerHeight();
                }
        ...
        _handleDelete: function(e) {
            ...
            activeDescendant.find(".sol-quick-delete").trigger("click");
            e.preventDefault();
        },
        ...
        _getDefaultFocusElement: function(fromBelow) {
            // update the hidden item count, if necessary
            this._updateExpandSelectedItemsButtonText(true);

            var elements = this._getFocusableElements();
            return fromBelow ? elements.pop() : elements.shift();
        },
        ...
        _renderOption: function (solOption, index, length, $optionalTargetContainer) {
            ...
            if (this.config.multiple) {
                // use checkboxes
                var optionCheckboxString = ('<input type="checkbox" id={0} class="sol-checkbox" name={0} tabindex="-1">').split('{0}').join(this.config.key + "_checkbox");
            ...
            $inputElement
                ...
                .on('change', function (event, check) {
                    ...
                    if (self.$selectedItemsContainer.hasClass("sol-selecteditems-expanded")) {
            ...
            $label = $(labelString)
                        ...
                        .attr('aria-label', solOption.tooltip)
                        .append($inputElement)
                        .append($labelText);
        ...
        // iterate through the selected item collection, assigning a `sol-visible` class to
        ...
        _updateSelectedItemsVisibility: function() {
            ...
            $selectedItems.each(function(idx, el) {
                ...
                if (minOffsetTop === elOffsetTop) {
                    ...
                    // should be hidden
                    el.classList.remove("sol-visible");
                }
            });
        },
        ...
        _getVisibleSelectedItems: function() {
            return this.$currentSelectionContainer.children(".sol-selected-display-item.sol-visible");
        },
        ...
        _getHiddenSelectedItems: function() {
            return this.$currentSelectionContainer.children(".sol-selected-display-item:not(.sol-visible)");
        },
        ...
        _getDropdownItems: function() {
            ...
            var items = this.$selectionContainer.find(".sol-option");
        ...
        _getFilteredDropdownItems: function() {
            ...
            var filteredItems = items.filter(".sol-option:not(.sol-filtered-search)");
            return filteredItems.length ? filteredItems : null;
        },

        _getFilteredDropdownItemCheckboxes: function() {
            // return the results of the current filter (i.e. visible items; sol-filtered-search items are hidden)
            var items = this._getFilteredDropdownItems();
            if (!items.length) {
                return null;
            }

            var filteredItems = items.find(".sol-checkbox:not(.sol-filtered-search)");
            return filteredItems.length ? filteredItems : null;
        },
        ...
        _getDeselectButtons: function() {
            if (this._getSelectedItemsCount()) {
                var items = this._getSelectedItems();
                var buttons = items.find(".sol-quick-delete");
        ...
        _getLastDeselectButton: function() {
            if (this._getSelectedItemsCount()) {
                ...
                if (this.$selectedItemsContainer.hasClass("sol-selecteditems-expanded")) {
                    ...
                    lastButton = lastVisibleSelectedItem.find(".sol-quick-delete");
                }
        ...
        _getViewmodeText: function() {
            ...
            var selectedItems = this.$currentSelectionContainer.children(".sol-selected-display-item");
            var count = selectedItems.length;
            var text = this.config.texts.noItemsSelected;
            var separator = this.config.texts.viewmodeItemsSeparator;

            if (count > 0) {
                text = $(selectedItems, ".sol-selected-display-item-text").map(function() {
                    return $(this).text();
                }).get().join(separator);
            }
        ...
        _collapseViewmodeContainer: function() {
            ...
            this.$viewmodeContainer.removeClass("sol-viewmode-expanded");
        },
        ...
        _checkViewmodeOverflow: function() {
            ...
            if (viewmodeTextElement.scrollWidth === viewmodeTextElement.offsetWidth) {
                this.$viewmodeButtonContainer.addClass("sol-hidden");
            } else {
                this.$viewmodeButtonContainer.removeClass("sol-hidden");
            }
        },

        _updateSelectedItemsContainerClass: function() {
            var noSelectionClass = "sol-none-selected";
            if (this._getSelectedItemsCount() === 0) {
        ...
        _updateExpandSelectedItemsButtonText: function(force) {
            ...
            if (!force && !this.$container.hasClass("sol-active")) {
            ...
            if (hiddenItemCount > 0) {
                this.$selectedItemsToggleButton
                    ...
                    .removeClass("sol-invisible"); // the button should be (initially) hidden
            } else {
                this.$selectedItemsToggleButton.addClass("sol-invisible"); // hide the button again
            }
        },
        ...
        _selectionChange: function ($changeItem, index, skipCallback) {
            var itemLabel = $changeItem.siblings(".sol-optionitem-text").text();
        ...
        _addSelectionDisplayItem: function ($changedItem, index) {
            ...
            if (!$existingDisplayItem) {
                ...
                var displaySelectedItemString = ('<li class="sol-selected-display-item sol-visible" />');
                ...
                if ((this.config.multiple || this.config.allowNullSelection)) {
                    $('<button class="sol-quick-delete" tabindex="-1"><span class="sol-glyph" /></button>')
                        ...
                        .on("click", function () {
                            // when this item is remove, we'll need to activate to the next item
        ...
        _setNoResultsItemVisible: function (visible) {
            if (visible) {
                ...
                this.$noResultsItem.removeClass("sol-hidden");

                if (this.$actionButtons) {
                    this.$actionButtons.addClass("sol-hidden");
                }

                this.$selection.addClass('sol-selection-hidden');

            } else {
                this.$noResultsItem.addClass("sol-hidden");

                if (this.$actionButtons) {
                    this.$actionButtons.removeClass("sol-hidden");
                }
        ...
        isOpen: function () {
            return this.$container.hasClass('sol-open');
        },
        ...
        open: function () {
            if (this.isClosed()) {
                this.$innerContainer.toggleClass("sol-fullscreen", this.config.fullscreenCombobox);
                $("body").toggleClass("sol-fullscreen", this.config.fullscreenCombobox);

                this.$container.addClass('sol-open');
        ...
        close: function (skipReset) {
            if (this.isOpen()) {
                this.$innerContainer.removeClass("sol-fullscreen");
                $("body").removeClass("sol-fullscreen");

                // scroll the options to the top *before* we hide it
                // otherwise it won't work
                this.$selection.scrollTop(0);

                this.$container.removeClass("sol-open sol-compact");
        ...
        selectAll: function () {
            if (this.config.showSelectAll && this.config.multiple) {
                var $changedInputs = this.getNonFilteredOptions()
                    .find('input[type="checkbox"]:not([disabled], :checked):not(.sol-selectall)')
                    .trigger('change', true);
        ...
        getValues: function () {
            var selectedValues = [];
            this.$currentSelectionContainer.find('.sol-selected-display-item').each(function() {
        ...
        toggleControlState: function (isControlDisabled) {
            ...
            if (this.config.isControlDisabled) {
                ...
                this.$container.addClass('sol-disabled');

                // populate the viewmode container
                this._populateViewmodeContainer();
            } else {
                // remove the disabled class
                this.$container.removeClass('sol-disabled');
        ...
        getNonFilteredOptions: function() {
            return this.$selectionContainer.find('.sol-option:not(.sol-filtered-search):not(.sol-selectall)');
        },
