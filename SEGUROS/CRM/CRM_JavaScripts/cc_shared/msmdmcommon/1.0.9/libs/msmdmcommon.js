...
(function (MktSvcCommon) {
	...
	var CdnEnabledControlConfigProvider = (function () {
		function CdnEnabledControlConfigProvider(clientBaseUrl, webResourceRootNamespace, controlFolder, cdnConfigurationService) {
		...
		CdnEnabledControlConfigProvider.prototype.getLatestCdnBasePath = function (callback) {
			...
			this.cdnConfigurationService.loadCdnConfiguration(function (result) {
				...
				var cdnRoot = result.PublicAssetsCdn;
				var controlRoot = cdnEnabled === "true" ? result.PublicAssetsCdn : _this.getLocalCrmWebResourcesPath();
...
(function (MktSvcCommon) {
	"use strict";
	var CdnConfigurationService = (function () {
		function CdnConfigurationService(crmUrlBase, ajaxHelper, logger) {
			this.cdnConfigurationEntityPluralName = "msdyncrm_cdnconfigurations";
			this.crmUrlBase = crmUrlBase;
			this.ajaxHelper = ajaxHelper;
			this.logger = logger;
		}
		CdnConfigurationService.prototype.loadCdnConfiguration = function (successCallback) {
			var _this = this;
			var configurationUrl = this.resolveUrl(this.cdnConfigurationEntityPluralName);
			...
			this.ajaxHelper.GetRequest(configurationUrl, function (response) {
				...
				try {
					...
					successCallback(defaultCdnConfiguration);
					_this.logger.logException(MktSvc.Controls.Common.TraceLevel.Error, "MktSvcCommon.CdnConfigurationService", exc, new MktSvc.Controls.Common.Dictionary({ "ErrorMessage": "Error parsing cdn configuration.", "ErrorDetails": response }));
					return;
				}
			}, function (response) {
				successCallback(defaultCdnConfiguration);
				_this.logger.log(MktSvc.Controls.Common.TraceLevel.Error, "MktSvcCommon.CdnConfigurationService", new MktSvc.Controls.Common.Dictionary({ "ErrorMessage": "Error loading cdn configuration", "ErrorDetails": response }));
			});
		};
		CdnConfigurationService.prototype.resolveUrl = function (entity) {
			var entityRecordUrlBuilder = new MktSvc.Controls.Common.EntityRecordUrlBuilder(this.crmUrlBase, this.cdnConfigurationEntityPluralName);
			var attributes = ["msdyncrm_cdnenabled", "msdyncrm_publicassetscdn"];
			entityRecordUrlBuilder.setSelectedFields(attributes);
			var url = entityRecordUrlBuilder.build();
			return url;
		};
...
(function (MktSvcCommon) {
	...
	(function (EventArgs) {
		"use strict";
		var GalleryImageSelectedEventArgs = (function () {
...
(function (MktSvcCommon) {
	...
	(function (ImagePicker) {
		...
		var ImagePickerCustomControl = (function () {
			/**
			...
			ImagePickerCustomControl.prototype.open = function (containerId) {
				...
				// Append the image picker container to the gallery
				var imagePickerContainer = $("<div>")
                    .attr("id", this.imagePickerContainerId)
                    .appendTo(container);
			...
			ImagePickerCustomControl.prototype.generateChildListeners = function (context) {
				...
				// TODO: popupRendered should be disposed as well
			...
			ImagePickerCustomControl.prototype.onImagePickerItemSelected = function (parameters) {
				this.galleryControlCallback(parameters.url);
			};
			/**
             ...
             */
			ImagePickerCustomControl.prototype.galleryControlCallback = function (imageUrl) {
...
(function (MktSvcCommon) {
	...
	(function (ImagePicker) {
		"use strict";
		var ImagePickerCustomControlFactory = (function () {
			/**
             * Initializes a new instance of ImagePickerCustomControl
             * @param context The bag of values currently assigned to the control's manifest parameters
             */
...
(function (MktSvcCommon) {
	...
	(function (Lookup) {
		...
		var LookupCustomControl = (function () {
             ...
             */
			function LookupCustomControl(context, logger, invalidRecordItemName, disableMru) {
             ...
             */
			LookupCustomControl.prototype.loadLookup = function (container, entityName, viewId, callback, initialValue, entitySetName, titleFieldName, bindDivElement, filter) {
				var _this = this;
				...
				var lookupCallback = function (updatedValue) {
					_this.loadLookup(container, entityName, viewId, callback, updatedValue ? { Id: updatedValue.Id || updatedValue.id, Name: updatedValue.Name || updatedValue.name } : null, entitySetName, titleFieldName, bindDiv);
				...
				if (!Object.isNullOrUndefined(initialValue)) {
					if (MktSvc.Controls.Common.String.isNullUndefinedOrWhitespace(initialValue.Name)) {
						...
						oDataClient.getData(oDataUrl.build()).done(function (result) {
							...
							_this.context.utils.bindDOMElement(_this.generateChildLookupControlControl(_this.controlId, entityName, viewId, lookupCallback, initialEntity, filter), bindDiv);
							...
							var logData = new MktSvc.Controls.Common.Dictionary({ Url: oDataUrl.build(), Status: status });
							logData.addOrUpdate(MktSvc.Controls.Common.ParameterKeys.Message, response);
							_this.logger.log(traceLevel, logEventName, logData);
							if (status === 404) {
						...
						this.context.utils.bindDOMElement(this.generateChildLookupControlControl(this.controlId, entityName, viewId, lookupCallback, initialEntity, filter), bindDiv);
					}
				}
				else {
					this.context.utils.bindDOMElement(this.generateChildLookupControlControl(this.controlId, entityName, viewId, lookupCallback, initialEntity, filter), bindDiv);
             ...
             */
			LookupCustomControl.prototype.generateChildLookupControlControl = function (controlId, entityName, viewId, callback, initialEntity, filter) {
...
(function (MktSvcCommon) {
	"use strict";
	var VirtualControlLoadDecorator = (function () {
