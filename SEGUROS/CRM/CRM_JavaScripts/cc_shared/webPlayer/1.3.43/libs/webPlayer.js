...
var SDKOrchestrator = /** @class */ (function () {
    ...
    SDKOrchestrator.prototype._resizeHostIframe = function (resizeInfo, messageType) {
        switch (messageType) {
            case ("host"):
                if (resizeInfo === true) {
    ...
    SDKOrchestrator.prototype._createIframe = function (iframeId, iframeUrl) {
        ...
        if (!navigator.userAgent.match('Edge')) {
            ...
            var sandboxProps = ["allow-popups", "allow-popups-to-escape-sandbox", "allow-same-origin", "allow-scripts", "allow-forms", "allow-orientation-lock", "allow-downloads"];
...
/******/ });
//# sourceMappingURL=C:/MainRepos/PA-Client/obj/Assets/js/WebPlayerSDKOrchestrator/WebPlayerSDKOrchestrator.js.map
﻿"use strict";
            ...
            CoreEmbedding.prototype._getHostName = function () {
                ...
                switch (this.embeddingOptions.endpoint) {
                    case Embedding.Endpoint.PROD:
                        endpoint = "https://apps.powerapps.com/";
                        break;
                    case Embedding.Endpoint.GCCMODERATE:
                        endpoint = "https://apps.gov.powerapps.us/";
                        break;
                    case Embedding.Endpoint.GCCHIGH:
                        endpoint = "https://apps.high.powerapps.us/";
                        break;
                    case Embedding.Endpoint.INT:
                        endpoint = "https://apps.int.powerapps.com/";
                        break;
                    case Embedding.Endpoint.TEST:
                        endpoint = "https://apps.test.powerapps.com/";
                        break;
                    case Embedding.Endpoint.DEV:
                        endpoint = "https://apps.local.powerapps.com:44329/";
                        break;
                    default:
                        endpoint = "https://apps.powerapps.com/";
                        break;
...
(function (WebPlayer) {
    var Sdk;
    (function (Sdk) {
        var EmbeddingConsentFlowHelper;
        (function (EmbeddingConsentFlowHelper) {
            var config;
            var appResources;
            var isInitialized;
            function initializeConsentService(consentServiceConfig, input) {
                var logger = consentServiceConfig.logger, renderConsentPage = consentServiceConfig.renderConsentPage;
                if (!isInitServicePayloadValid(input)) {
                    // Contract broken !  Do nothing.
                    return;
                }
                config = consentServiceConfig;
                appResources = input.data.appResourcePayload;
                isInitialized = true;
                logger.logOperationStart("WebPlayerRuntime.LoadingDisambiguationWithConsentService", input.data.initConsentServiceTimeStamp);
                renderConsentPage();
            }
            EmbeddingConsentFlowHelper.initializeConsentService = initializeConsentService;
            function isInitServicePayloadValid(input) {
                // please make sure the contract on the WebPlayer side has the same definition in here otherwise it would break the contract
                return input &&
                    input.data &&
                    input.data.appResourcePayload &&
                    typeof input.data.initConsentServiceTimeStamp === "number";
            }
            function onConsentServiceLoaded(input) {
                if (!isInitialized || !isOnServiceLoadedPayloadValid(input)) {
                    // Helper is not initialized yet or contract Broken ! Do nothing
                    return;
                }
                config.logger.logOperationEnd("WebPlayerRuntime.LoadingDisambiguationWithConsentService", input.data.consentDialogLoadedTimestamp, input.appId, input.containerId);
            }
            EmbeddingConsentFlowHelper.onConsentServiceLoaded = onConsentServiceLoaded;
            function isOnServiceLoadedPayloadValid(input) {
                return input &&
                    input.data &&
                    typeof input.data.consentDialogLoadedTimestamp === "number" &&
                    input.data.source === "AppPermissionDialog";
            }
            function sendPreloadDataToConsentService() {
                if (!isInitialized) {
                    return;
                }
                var params = { api: Sdk.PowerAppsEmbeddingCallbackApi.GetPreloadConsentServiceData, data: { appResources: appResources } };
                config.postMessageToConsentService(params);
                appResources = undefined;
            }
            EmbeddingConsentFlowHelper.sendPreloadDataToConsentService = sendPreloadDataToConsentService;
        })(EmbeddingConsentFlowHelper = Sdk.EmbeddingConsentFlowHelper || (Sdk.EmbeddingConsentFlowHelper = {}));
...
(function (WebPlayer) {
    var Sdk;
    (function (Sdk) {
        var EmbeddingLogger = /** @class */ (function () {
            function EmbeddingLogger(logCallback) {
                this._metricsDictionary = {};
                this._logCallback = logCallback;
            }
            EmbeddingLogger.prototype.logOperationStart = function (operationName, startTimeStamp) {
                if (this._metricsDictionary.hasOwnProperty(operationName) && startTimeStamp) {
                    return;
                }
                this._metricsDictionary[operationName] = {
                    startTimeStamp: startTimeStamp,
                    endTimeStamp: 0
                };
            };
            EmbeddingLogger.prototype.logOperationEnd = function (operationName, endTimeStamp, appId, containerId) {
                if (!this._metricsDictionary.hasOwnProperty(operationName) && endTimeStamp) {
                    return;
                }
                this._metricsDictionary[operationName].endTimeStamp = endTimeStamp;
                var duration = this._metricsDictionary[operationName].endTimeStamp - this._metricsDictionary[operationName].startTimeStamp;
                this._logCallback({ operationName: operationName, duration: duration }, appId, containerId);
                delete this._metricsDictionary[operationName];
            };
            return EmbeddingLogger;
        }());
        Sdk.EmbeddingLogger = EmbeddingLogger;
    })(Sdk = WebPlayer.Sdk || (WebPlayer.Sdk = {}));
})(WebPlayer || (WebPlayer = {}));
//!
//! Copyright (C) Microsoft Corporation.  All rights reserved.
//!
// This is a simple encoder for the hash result coming from the parent
// took it apart from parser to make file smaller
var WebPlayer;
(function (WebPlayer) {
    var Sdk;
    (function (Sdk) {
        var HashDataHandler = /** @class */ (function () {
            function HashDataHandler() {
            }
            HashDataHandler.getHashFromUrl = function (fullUrl) {
                if (!fullUrl) {
                    return "";
                }
                var hashIndex = fullUrl.indexOf("#");
                if (hashIndex > 0) {
                    // this means we have more data after hash, an object should be searched
                    return fullUrl.substr(hashIndex + 1);
                }
                else {
                    return "";
                }
            };
            HashDataHandler.appendHashToUrl = function (currentUrl, hashToReplace) {
                if (!currentUrl || !hashToReplace) {
                    return currentUrl;
                }
                var hashIndex = currentUrl.indexOf("#");
                // next line tries to find the # in the url. If it has the hash, it will grab the url until
                // the # and replace whatever comes next by the new hash data.
                // If however hash isn't present, it will simply add the # and append to it.
                var finalUrlToReturn = (hashIndex > 0 ? currentUrl.substr(0, hashIndex + 1) : (currentUrl + "#")) + hashToReplace;
                return finalUrlToReturn;
            };
            HashDataHandler.addAccessTokenToHash = function (accessToken, parentRequestId, currentHash) {
                if (!accessToken || currentHash === null || currentHash === undefined) {
                    return currentHash;
                }
                var hashObject = {};
                try {
                    ...
                    // if we got here, it's because the data after hash couldn't be parsed to an object
                    // can't log anything useful here
                }
                if (typeof (hashObject) === "object") {
                    // If it has one, will be overwritten. If not it will be added.
                    hashObject[this._accessTokenKey] = accessToken;
                    hashObject[this._parentRequestIdKey] = parentRequestId || "";
                    var accessTokenObjectToAppend = encodeURIComponent(JSON.stringify(hashObject));
                    return accessTokenObjectToAppend;
                }
                else {
            ...
            HashDataHandler.validateUrlWasntCorrupted = function (originalUrl, finalUrl) {
                var hashIndex = originalUrl.indexOf("#");
                // next two lines test if the base url (the url up to the #) is preserved after the changes
                // this is to validate that we do not break the original url
                var baseUrlLength = hashIndex > 0 ? hashIndex : originalUrl.length;
                if (finalUrl.substr(0, baseUrlLength).indexOf(originalUrl.substr(0, baseUrlLength)) >= 0) {
                    return finalUrl;
                }
                else {
                    // something went wrong. To avoid unexpected behavior, we give up and return the original url.
                    return originalUrl;
                }
            };
            HashDataHandler.setTokenAsHashParam = function (originalUrl, token, parentRequestId) {
                if (!originalUrl) {
                    return originalUrl;
                }
                var hashFromOriginalUrl = HashDataHandler.getHashFromUrl(originalUrl);
                var newHashAfterAccessTokenIsAdded = HashDataHandler.addAccessTokenToHash(token, parentRequestId, hashFromOriginalUrl);
                var finalUrlWithHash = HashDataHandler.appendHashToUrl(originalUrl, newHashAfterAccessTokenIsAdded);
                return this.validateUrlWasntCorrupted(originalUrl, finalUrlWithHash);
            };
            HashDataHandler._accessTokenKey = "access_token";
            HashDataHandler._parentRequestIdKey = "pid";
            return HashDataHandler;
        }());
        Sdk.HashDataHandler = HashDataHandler;
    })(Sdk = WebPlayer.Sdk || (WebPlayer.Sdk = {}));
})(WebPlayer || (WebPlayer = {}));
//!
//! Copyright (C) Microsoft Corporation.  All rights reserved.
...
(function (WebPlayer) {
    ...
    (function (Sdk) {
        ...
        (function (PowerAppsEmbeddingCallbackApi) {
            ...
            PowerAppsEmbeddingCallbackApi[PowerAppsEmbeddingCallbackApi["GetPreloadConsentServiceData"] = 48] = "GetPreloadConsentServiceData";
        ...
        var PowerAppsEmbedding = /** @class */ (function (_super) {
            ...
            function PowerAppsEmbedding(options, hostWindow) {
                ...
                if (options.consentContainerId) {
                    var consentContainer = document.getElementById(options.consentContainerId);
                    if (!consentContainer) {
            ...
            PowerAppsEmbedding.prototype._constructUrl = function (embeddingOptions, isConsentService) {
                if (embeddingOptions === void 0) { embeddingOptions = this.options; }
                if (isConsentService === void 0) { isConsentService = false; }
                var url = "";
                // Standalone consent dialog aka Consent Service is only supported on Service Fabric endpoint
                var isServiceFabric = isConsentService ? true : this._isCallingServiceFabricEndpoint();
                if (isServiceFabric) {
                    // strip the prefix from the url for service fabric
                    var appGuid = decodeURIComponent(embeddingOptions.appId).replace(this.appIdConst, "");
                    var routeName = isConsentService ? "consent" : "play";
                    if (embeddingOptions.appEnvironmentId) {
                        if (embeddingOptions.logicalAppName) {
                            url = "" + embeddingOptions.hostName + routeName + "/e/" + embeddingOptions.appEnvironmentId + "/an/" + embeddingOptions.logicalAppName;
                        }
                        else {
                            url = "" + embeddingOptions.hostName + routeName + "/e/" + embeddingOptions.appEnvironmentId + "/a/" + appGuid;
                        }
                    }
                    else {
                        // default url
                        url = "" + embeddingOptions.hostName + routeName + "/" + appGuid;
                    }
                    url = this.appendCoreEmbeddingParams(url);
                    var authMode = "default";
                    if (embeddingOptions.enableAuthFlowProxy) {
                        authMode = "authflowproxy";
                    }
                    else if (embeddingOptions.getAccessToken) {
                        if (embeddingOptions.enableOnBehalfOf) {
                            authMode = "onbehalfof";
                        }
                        else {
                            authMode = "override";
                    ...
                    if (!isConsentService && embeddingOptions.standAloneConsent) {
                        url += "&standAloneConsent=true";
                    }
                    if (isConsentService && typeof embeddingOptions.openConnectorAuthPopup === "function") {
                    ...
                    if (embeddingOptions.getAccessToken) {
                        webPlayerAppSegment = "embedapp";
                    }
                    url = this.constructBaseUrl(isServiceFabric, webPlayerAppSegment);
                    // only add an & if there is an existing query
                    var delimiter = url.indexOf("?") !== -1 ? "&" : "?";
                    var encodedAppId = encodeURIComponent(embeddingOptions.appId);
                    url = "" + url + delimiter + "appId=" + encodedAppId;
                }
                url += "&hideNavBar=true";
                if (embeddingOptions.tenantId) {
                    url += "&tenantId=" + embeddingOptions.tenantId;
                }
                if (embeddingOptions.appInfo && embeddingOptions.appInfo.properties && embeddingOptions.appInfo.properties.backgroundColor) {
                    url += "&screenColor=" + encodeURIComponent(embeddingOptions.appInfo.properties.backgroundColor);
                }
                else if (embeddingOptions.splashScreenCssColor) {
                    url += "&screenColor=" + encodeURIComponent(embeddingOptions.splashScreenCssColor);
                }
                if (embeddingOptions.delegateLaunchUrl) {
                    url += "&delegatelaunchurl=true";
                }
                if (embeddingOptions.hideAppSplashScreen) {
                    url += "&hideAppSplash=true";
                }
                if (embeddingOptions.packagePropertiesJson) {
                    url += "&packageProperties=" + encodeURIComponent(embeddingOptions.packagePropertiesJson);
                }
                if (embeddingOptions.cachedFilesJson) {
                    url += "&skipIframeCreation=true";
                }
                url += "&sdkVersion=" + this.sdkVersion;
                url += "&iframeContainerId=";
                if (isConsentService) {
                    url += embeddingOptions.consentContainerId;
                }
                else {
                    url += embeddingOptions.containerId;
                }
                if (embeddingOptions.locale) {
                    url += "&locale=" + embeddingOptions.locale;
                }
                // Need to clear this.params before assigning a value since constructUrl is called multiple times
                this.params = "";
                if (embeddingOptions.parameters) {
                    for (var paramKey in embeddingOptions.parameters) {
                        if (embeddingOptions.parameters.hasOwnProperty(paramKey)) {
                            this.params += "&" + paramKey + "=" + encodeURIComponent(embeddingOptions.parameters[paramKey]);
                        }
                    }
                    url += this.params;
                }
                if (embeddingOptions.useIntegrationEnvironment) {
                    url += "&overrideEnvironment=Tip1&overridePackager=Int";
                }
                if (embeddingOptions.embedderOrigin) {
                    url += "&embedderOrigin=" + embeddingOptions.embedderOrigin;
                }
                return url;
            };
            PowerAppsEmbedding.prototype._generateIframeSize = function (embeddingOptions) {
                if (embeddingOptions === void 0) { embeddingOptions = this.options; }
                if (embeddingOptions.appInfo && embeddingOptions.appInfo.tags && embeddingOptions.appInfo.tags.primaryDeviceWidth && embeddingOptions.appInfo.tags.primaryDeviceHeight) {
                    var aspectRatio = embeddingOptions.appInfo.tags.primaryDeviceHeight / embeddingOptions.appInfo.tags.primaryDeviceWidth;
                    if (aspectRatio > 0) {
                        // Aspect ratio is a valid value
                        if (embeddingOptions.maxWidth) {
                            // Max width is set
                            var width = parseInt(embeddingOptions.maxWidth, 10);
                            var height = width * aspectRatio;
                            if (embeddingOptions.maxHeight) {
                                // max height is set
                                var maxHeight = parseInt(embeddingOptions.maxHeight, 10);
                                if (height > maxHeight) {
                            ...
                            if (embeddingOptions.maxHeight) {
                                ...
                                var height = parseInt(embeddingOptions.maxHeight, 10);
                        ...
                        return this._generateIframeSizeWithNoAspectRatio(embeddingOptions);
                    }
                }
                else {
            ...
            PowerAppsEmbedding.prototype._generateIframeSizeWithNoAspectRatio = function (embeddingOptions) {
                if (embeddingOptions === void 0) { embeddingOptions = this.options; }
                var height;
                var width;
                if (embeddingOptions.maxWidth) {
                ...
                if (embeddingOptions.maxHeight) {
                    // max width is set so force the width to be that
                    height = parseInt(embeddingOptions.maxHeight, 10) + "px";
                }
                else {
                    // Set to to fill 100% of container div
                    height = "100%";
            ...
            PowerAppsEmbedding.prototype._renderAppWithSdkOrchestrator = function (sdkMetadata) {
                ...
                this.queuedCdpData = [];
                this._initEmbeddingLogger();
                this._updateWebPlayerInstances();
            };
            ...
            PowerAppsEmbedding.prototype._renderWebPlayerApp = function () {
                ...
                this.iframe.name = this._generateIframeName("webPlayer-iFrame");
            ...
            PowerAppsEmbedding.prototype._updateWebPlayerInstances = function () {
                if (this.options.standAloneConsent) {
                    if (!PowerAppsEmbedding.consentInstances) {
                        PowerAppsEmbedding.consentInstances = new Array();
                    }
                    PowerAppsEmbedding.consentInstances.push({
                        embeddingOptions: this.options,
                        webPlayerIframeName: this.iframe.name,
                        isConsentPending: false
                    });
                }
            };
            PowerAppsEmbedding.prototype._generateIframeName = function (rootName) {
                var counter = 0;
                var iframeName;
                do {
                    counter++;
                    iframeName = rootName + "-" + counter;
                } while (document.getElementsByName(iframeName).length > 0);
                return iframeName;
            };
            PowerAppsEmbedding.prototype._initEmbeddingLogger = function () {
                this.embeddingLogger = new Sdk.EmbeddingLogger(function (data, appId, containerId) {
                    var params = {
                        api: Sdk.PowerAppsEmbeddingCallbackApi.Log,
                        data: data
                    };
                    var iframe;
                    for (var index = 0; index < PowerAppsEmbedding.consentInstances.length; index++) {
                        var webPlayerInstance = PowerAppsEmbedding.consentInstances[index];
                        if (webPlayerInstance.embeddingOptions.appId === appId && webPlayerInstance.embeddingOptions.consentContainerId === containerId) {
                            iframe = document.getElementsByName(webPlayerInstance.webPlayerIframeName)[0];
                            break;
                        }
                    }
            ...
            PowerAppsEmbedding.prototype._renderConsentPage = function () {
                var _a, _b;
                if (!PowerAppsEmbedding.consentInstanceInSession) {
                    return;
                }
                (_b = (_a = PowerAppsEmbedding.consentInstanceInSession.embeddingOptions).consentDialogOpenCloseHandler) === null || _b === void 0 ? void 0 : _b.call(_a, true);
                var url = this._constructUrl(PowerAppsEmbedding.consentInstanceInSession.embeddingOptions, true);
                PowerAppsEmbedding.consentIframe = this._createFrame(url, PowerAppsEmbedding.consentInstanceInSession.embeddingOptions);
                PowerAppsEmbedding.consentIframe.name = this._generateIframeName("consentService-iFrame");
                PowerAppsEmbedding.consentContainerElement = document.getElementById(PowerAppsEmbedding.consentInstanceInSession.embeddingOptions.consentContainerId);
                PowerAppsEmbedding.consentContainerElement.appendChild(PowerAppsEmbedding.consentIframe);
            };
            PowerAppsEmbedding.prototype._createFrame = function (url, embeddingOptions) {
                if (embeddingOptions === void 0) { embeddingOptions = this.options; }
                var iframeElement = document.createElement("iframe");
                ...
                var iframeDimensions = this._generateIframeSize(embeddingOptions);
            ...
            PowerAppsEmbedding.prototype.dispose = function () {
                ...
                this._disposeConsentPage(true);
                this.eventListeners = {};
                window.removeEventListener("message", this._onMessageReceived);
            };
            PowerAppsEmbedding.prototype._disposeConsentPage = function (clearConsentInstances) {
                var _a, _b;
                if (PowerAppsEmbedding.consentInstanceInSession) {
                    PowerAppsEmbedding.consentContainerElement.removeChild(PowerAppsEmbedding.consentIframe);
                    PowerAppsEmbedding.consentIframe = null;
                    (_b = (_a = PowerAppsEmbedding.consentInstanceInSession.embeddingOptions).consentDialogOpenCloseHandler) === null || _b === void 0 ? void 0 : _b.call(_a, false);
                    PowerAppsEmbedding.consentInstanceInSession = null;
                    if (clearConsentInstances) {
                        PowerAppsEmbedding.consentInstances = null;
                    }
                }
            };
            PowerAppsEmbedding.prototype._initializeNextConsent = function () {
                // Look for the next Canvas app that needs consent from the user
                for (var index = 0; index < PowerAppsEmbedding.consentInstances.length; index++) {
                    var consentInstance = PowerAppsEmbedding.consentInstances[index];
                    if (consentInstance.isConsentPending) {
                        // Canvas app that needs consent is found. Reload the Web Player iframe to restart the standalone consent process.
                        consentInstance.isConsentPending = false;
                        var webPlayerIframe = document.getElementsByName(PowerAppsEmbedding.consentInstances[index].webPlayerIframeName)[0];
                        webPlayerIframe.src = webPlayerIframe.src;
                        break;
                    }
                }
            };
            ...
            PowerAppsEmbedding.prototype._onMessageReceived = function (e) {
                ...
                var input = e.data;
                if (input && input.api !== undefined && (!input.appId || input.appId.toLowerCase() === this.options.appId.toLowerCase()) &&
                    (!input.containerId || input.containerId === this.options.containerId || input.containerId === this.options.consentContainerId)) {
                    var eventName = "";
                    switch (input.api) {
                        case Sdk.PowerAppsEmbeddingCallbackApi.AppLoaded:
                            eventName = "load";
                            this._removeConsentInstance(input.appId, input.containerId);
                            break;
                        ...
                        case Sdk.PowerAppsEmbeddingCallbackApi.InteractionRequired:
                            ...
                            if (PowerAppsEmbedding.consentInstanceInSession && input.appId && input.containerId &&
                                input.appId === PowerAppsEmbedding.consentInstanceInSession.embeddingOptions.appId &&
                                input.containerId === PowerAppsEmbedding.consentInstanceInSession.embeddingOptions.consentContainerId) {
                                Sdk.EmbeddingConsentFlowHelper.onConsentServiceLoaded(input);
                                // consentDialogUserInteractionHandler is only used in automated BVT at this time
                                (_b = (_a = PowerAppsEmbedding.consentInstanceInSession.embeddingOptions).consentDialogUserInteractionHandler) === null || _b === void 0 ? void 0 : _b.call(_a);
                            }
                            break;
                        case Sdk.PowerAppsEmbeddingCallbackApi.OpenConsentPage:
                            if (PowerAppsEmbedding.consentInstanceInSession) {
                                // There is a standalone consent dialog in session
                                for (var i = 0; i < PowerAppsEmbedding.consentInstances.length; i++) {
                                    var webPlayerInstance = PowerAppsEmbedding.consentInstances[i];
                                    if (input.appId === webPlayerInstance.embeddingOptions.appId &&
                                        input.containerId === webPlayerInstance.embeddingOptions.containerId &&
                                        !webPlayerInstance.isConsentPending) {
                                        // Found the Canvas app that requested standalone consent dialog
                                        // Set the isConsentPending flag to true
                                        webPlayerInstance.isConsentPending = true;
                                        break;
                                    }
                                }
                            }
                            else {
                                // There is no standalone consent dialog in session
                                var index = void 0;
                                for (index = 0; index < PowerAppsEmbedding.consentInstances.length; index++) {
                                    var consentInstance = PowerAppsEmbedding.consentInstances[index];
                                    if (input.appId === consentInstance.embeddingOptions.appId && input.containerId === consentInstance.embeddingOptions.containerId) {
                                        // Found the Canvas app that requested standalone consent dialog
                                        // Assign the value of the consentInstance that requires consent to the static variable of consentInstanceInSession
                                        PowerAppsEmbedding.consentInstanceInSession = {
                                            embeddingOptions: consentInstance.embeddingOptions,
                                            isConsentPending: false,
                                            webPlayerIframeName: consentInstance.webPlayerIframeName
                                        };
                                        break;
                                    }
                                }
                                if (index < PowerAppsEmbedding.consentInstances.length) {
                                    // Canvas app that requested standalone consent dialog has been found.
                                    // Need to remove the consent instance from the consent instances array
                                    PowerAppsEmbedding.consentInstances.splice(index, 1);
                                }
                                // Kick off the standalone consent dialog workflow
                                Sdk.EmbeddingConsentFlowHelper.initializeConsentService({
                                    logger: this.embeddingLogger,
                                    renderConsentPage: function () {
                                        _this._renderConsentPage();
                                    },
                                    postMessageToConsentService: function (params) {
                                        var _a, _b;
                                        (_b = (_a = PowerAppsEmbedding.consentIframe) === null || _a === void 0 ? void 0 : _a.contentWindow) === null || _b === void 0 ? void 0 : _b.postMessage(params, "*");
                                    }
                                }, input);
                            }
                            break;
                        case Sdk.PowerAppsEmbeddingCallbackApi.CloseConsentPage:
                            this._disposeConsentPage(false);
                            this._initializeNextConsent();
                            break;
                        case Sdk.PowerAppsEmbeddingCallbackApi.SetConsentResult:
                            ...
                            var webPlayerIframe = document.getElementsByName(PowerAppsEmbedding.consentInstanceInSession.webPlayerIframeName)[0];
                            webPlayerIframe.contentWindow.postMessage(consentResultParams, "*");
                            break;
                        case Sdk.PowerAppsEmbeddingCallbackApi.OpenConnectorAuthPopup:
                            (_e = (_c = PowerAppsEmbedding.consentInstanceInSession) === null || _c === void 0 ? void 0 : (_d = _c.embeddingOptions).openConnectorAuthPopup) === null || _e === void 0 ? void 0 : _e.call(_d, input.data, this._processConnectorAuthResult);
                            break;
                        case Sdk.PowerAppsEmbeddingCallbackApi.GetPreloadConsentServiceData:
                            Sdk.EmbeddingConsentFlowHelper.sendPreloadDataToConsentService();
                            break;
                        default:
                            // Invalid API.
                            break;
            ...
            PowerAppsEmbedding.prototype._removeConsentInstance = function (appId, containerId) {
                if (PowerAppsEmbedding.consentInstances && appId && containerId) {
                    var index = void 0;
                    for (index = 0; index < PowerAppsEmbedding.consentInstances.length; index++) {
                        var webPlayerInstance = PowerAppsEmbedding.consentInstances[index];
                        if (appId === webPlayerInstance.embeddingOptions.appId &&
                            (containerId === webPlayerInstance.embeddingOptions.containerId ||
                                containerId === webPlayerInstance.embeddingOptions.consentContainerId)) {
                            break;
                        }
                    }
                    if (index < PowerAppsEmbedding.consentInstances.length) {
            ...
            PowerAppsEmbedding.prototype._processConnectorAuthResult = function (authResult) {
                if (authResult && authResult.result) {
                    ...
                    PowerAppsEmbedding.consentIframe.contentWindow.postMessage(authResultParam, "*");
                }
            };
            ...
            PowerAppsEmbedding.prototype._isCallingServiceFabricEndpoint = function () {
                return (!!this.options.hostName && Sdk.UrlValidator.isServiceFabricEndpoint(this.options.hostName));
            };
            /** Override */
            PowerAppsEmbedding.prototype.setTokenAsHashParam = function (token, parentRequestId, frameOrigin) {
                var _a, _b;
                // We must check if the origin matches the iframe src, because the src won't be updated in case of redirect (e.g., apps.preview.powerapps.com)
                // If this check isn't made, the set below will trigger another redirect and this will end up in an infinite loop
                if (token && frameOrigin && ((_b = (_a = this.iframe) === null || _a === void 0 ? void 0 : _a.src) === null || _b === void 0 ? void 0 : _b.indexOf(frameOrigin)) === 0) {
                    this.iframe.src = Sdk.HashDataHandler.setTokenAsHashParam(this.iframe.src, token, parentRequestId);
                }
            };
...
//! Copyright (C) Microsoft Corporation.  All rights reserved.
//!
// Currently this is a simple class to facilitate better unit testing
// In the future, if we expand URL validation in the embedding SDK, this will facilitate code re-use
var WebPlayer;
(function (WebPlayer) {
    var Sdk;
    (function (Sdk) {
        var UrlValidator = /** @class */ (function () {
            ...
            UrlValidator.isServiceFabricEndpoint = function (endpoint) {
                return this._normalRegExp.test(endpoint) || this._dodRegExp.test(endpoint);
            };
            UrlValidator._normalRegExp = new RegExp("https:\/\/apps(\.[a-z0-9_]+)?\.powerapps\.(com|us|cn)", 'i');
            UrlValidator._dodRegExp = new RegExp("https:\/\/play\.apps\.appsplatform\.us", 'i');
            return UrlValidator;
        }());
        Sdk.UrlValidator = UrlValidator;
    })(Sdk = WebPlayer.Sdk || (WebPlayer.Sdk = {}));
})(WebPlayer || (WebPlayer = {}));
